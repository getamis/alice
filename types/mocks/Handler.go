...
func (_m *Handler) Finalize(logger log.Logger) (types.Handler, error) {
	...
	var r1 error
	if rf, ok := ret.Get(0).(func(log.Logger) (types.Handler, error)); ok {
		return rf(logger)
	}
...
func (_m *Handler) GetRequiredMessageCount() uint32 {
	...
	if len(ret) == 0 {
		panic("no return value specified for GetRequiredMessageCount")
	}
...
func (_m *Handler) IsHandled(logger log.Logger, id string) bool {
	...
	if len(ret) == 0 {
		panic("no return value specified for IsHandled")
	}
