...
func newMasterKeyFunc(startIndex int, garbleStart int, garbleEnd int, computeFunc computeOwnSeedBits, parseFunc parseResultFunc) func(peerManager types.PeerManager, sid []uint8, seed []uint8, function string) (*initial, error) {
	return func(peerManager types.PeerManager, sid []uint8, seed []uint8, path string) (*initial, error) {
		...
		garcir, garMsg, err := cir.Garbled(KAPPA, computeFunc(seedBits, seedRandom), circuit.EncryptFunc(startIndex))
		if err != nil {
			return nil, err
		}
