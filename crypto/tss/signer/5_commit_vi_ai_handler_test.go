...
var _ = Describe("commit vi ai handler, negative cases", func() {
	var (
		peerId = "peer-id"
	...
	BeforeEach(func() {
		signers, listeners = newTestSigners(func(p types.PeerManager) types.PeerManager {
			return newStopPeerManager(Type_CommitViAi, p)
		})
		...
		for _, s := range signers {
			for {
				_, ok := s.GetHandler().(*commitViAiHandler)
				if !ok {
	...
	Context("IsHandled", func() {
		It("peer not found", func() {
			for _, s := range signers {
				rh, ok := s.GetHandler().(*commitViAiHandler)
