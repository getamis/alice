...
type serverHandler0 struct {
	...
	newPasswordResponser *oprf.Responser
...
func newServerHandler0(publicKey *ecpointgrouplaw.ECPoint, peerManager types.PeerManager, bks map[string]*birkhoffinterpolation.BkParameter, k *big.Int, secret *big.Int) (*serverHandler0, error) {
	...
	newResponser, err := oprf.NewResponser()
	if err != nil {
		return nil, err
	}
	...
	return &serverHandler0{
		...
		newPasswordResponser: newResponser,
...
func (p *serverHandler0) HandleMessage(logger log.Logger, message types.Message) error {
	...
	if err != nil {
		logger.Warn("Failed to handle old password request", "err", err)
		return err
	}
	newPasswordRes, err := p.newPasswordResponser.Handle(user0.NewPasswordRequest)
	if err != nil {
		logger.Warn("Failed to handle new password request", "err", err)
		return err
	}
	...
	p.peerManager.MustSend(message.GetId(), &Message{
		...
		Body: &Message_Server0{
			Server0: &BodyServer0{
				...
				NewPasswordResponse: newPasswordRes,
