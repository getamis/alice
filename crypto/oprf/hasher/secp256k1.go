...
// Get the hash value of pw by rejectSampling. Use two hash functions to generate element. The value is generated by two hash values.
// ref: https://eprint.iacr.org/2009/340.pdf
func getHashValueByRejectSampling(pw []byte, fieldOrder *big.Int) (*big.Int, error) {
	hashsha256PW := pw
	hashblake2bPW := pw
	for i := 0; i < maxRetry; i++ {
		hashblakePWC := blake2b.Sum256(hashblake2bPW)
		hash256PWC := sha256.Sum256(hashsha256PW)
		hashblakePW := hashblakePWC[:]
		hash256PW := hash256PWC[:]
		hash256Value := new(big.Int).SetBytes(hash256PW)
		hashblakeValue := new(big.Int).SetBytes(hashblakePW)
		if utils.InRange(hash256Value, big0, fieldOrder) == nil && utils.InRange(hashblakeValue, big0, fieldOrder) == nil {
			result := new(big.Int).Add(hash256Value, hashblakeValue)
