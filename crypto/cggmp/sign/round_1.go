...
func (p *round1Handler) HandleMessage(logger log.Logger, message types.Message) error {
	...
	negBeta, countDelta, r, s, D, F, phiProof, err := mtaWithProofAff_g(p.own.ssidWithBk, peer, p.paillierKey, round1.KCiphertext, p.gamma, Gamma)
	if err != nil {
		return err
	}
	// psihat share proof: M(prove,Πaff-g,(sid,i),(Iε,Jε,Dˆj,i,Kj,Fˆj,i,Xi);(xi,βˆi,j,sˆi,j,rˆi,j)).
	negBetahat, countSigma, rhat, shat, Dhat, Fhat, psihatProof, err := mtaWithProofAff_g(p.own.ssidWithBk, peer, p.paillierKey, round1.KCiphertext, p.bkMulShare, p.bkpartialPubKey)
	if err != nil {
		return err
	}
	...
	G := pt.NewBase(curve)
	psipaiProof, err := paillierzkproof.NewKnowExponentAndPaillierEncryption(parameter, p.own.ssidWithBk, p.gamma, p.mu, p.gammaCiphertext, p.paillierKey.GetN(), peerPed.Getn(), peerPed.Gets(), peerPed.Gett(), Gamma, G)
	if err != nil {
		return err
	}
...
func mtaWithProofAff_g(ownssid []byte, peer *peer, paillierKey *paillier.Paillier, msgCipher []byte, x *big.Int, ecPoint *pt.ECPoint) (*big.Int, *big.Int, *big.Int, *big.Int, []byte, *big.Int, *paillierzkproof.PaillierAffAndGroupRangeMessage, error) {
	beta, s, r, D, F, err := performMTA(peer, paillierKey, msgCipher, x)
	if err != nil {
		return nil, nil, nil, nil, nil, nil, nil, err
	}
	peerPed := peer.para
	peerPaillierKey := peerPed.ToPaillierPubKeyWithSpecialG()
	proof, err := paillierzkproof.NewPaillierAffAndGroupRangeMessage(parameter, ownssid, x, beta, s, r, peerPed.Getn(), paillierKey.GetN(), new(big.Int).SetBytes(msgCipher), D, F, peerPed.Getn(), peerPed.Gets(), peerPed.Gett(), ecPoint)
	if err != nil {
		return nil, nil, nil, nil, nil, nil, nil, err
	}
	adjustBeat, count := computeBeta(beta, peerPaillierKey.GetN(), ecPoint.GetCurve().Params().N, big.NewInt(0))
